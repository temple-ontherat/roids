...
function setup() {
  ...
  u=0;
  rot=1; //test
  //rot=5; changed  '10' for old ASUS and see tri7.js rotation left/right changes
  textSize(40);
  ...
  for (i=0; i<12; i++) {
      ...
      if ((alien) && (alShot) && (alien.colorG==255)) { 
        for (let z=0; z<alShot.length; z++) {
            for (i=0; i<12; i++) {
                joe = p5.Vector.add(tri.pt[i], tri.pos);
                if ((joe.dist(alShot[z].pos)<2) && (tri.colorG=255)) {
        alShot=[];  
        crash=true;
        shipCrash();  
        crashShip();
      ...
      if ((alien) && (alien.colorG==255)) {
            for (i=0; i<12; i++) {
                joe = p5.Vector.add(tri.pt[i], tri.pos);
                if ((joe.dist(alien.pos)<15) && (tri.colorG=255)) {
